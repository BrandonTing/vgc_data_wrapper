...
import { getBasePower, type Move, type Pokemon, type Stat, type StatStage } from "./basePower"
...
test("correctly calculate base power for speed related moves", function () {
    let testFastPokemon = genTestMon({
        stat: {
            speed: 300
        }
    })
    ...
    let basePowerElectricBall = getBasePower(testFastPokemon, testSlowPokemon, testElectricBall);
    expect(basePowerElectricBall).toBe(150)
    
    testFastPokemon.stat.speed = 175
    basePowerElectricBall = getBasePower(testFastPokemon, testSlowPokemon, testElectricBall);
    ...
    let basePowerGyroBall = getBasePower(testSlowPokemon, testFastPokemon, testGyroBall);
    expect(basePowerGyroBall).toBe(150)

    testFastPokemon.stat.speed = 200
    basePowerGyroBall = getBasePower(testSlowPokemon, testFastPokemon, testGyroBall);
    expect(basePowerGyroBall).toBe(100)

})

test("correctly calculate base power for Grass knot and Low kick", function () {
    const testPokemon = genTestMon()
    ...
    let testHaavyPokemon = genTestMon({
        weight: 999
    })
    let basePowerLowKick = getBasePower(testPokemon, testHaavyPokemon, lowKick)
    let basePowerGrassknot = getBasePower(testPokemon, testHaavyPokemon, grassKnot)
...
test("correctly calculate base power for heavy slam & heat crash", function () {
    const testDefender = genTestMon({
        weight: 10
    })
    let testAttacker = genTestMon({
        weight: 15
    })
...
test("base power of weather ball", function () {
...
test("base power of tera blast", function () {
    const testMon = genTestMon();
...
test("base power of Power Trip & Stored Power", function () {
    const testMon = genTestMon({
        statStage: {
            attack: 6,
            defense: 5,
