import { typeAttackEffectivenessMap, type Type } from "./config";
...
const statProps = ["hp","attack", "defense", "specialAttack", "specialDefense", "speed"] as const;
...
export function getBasePower(attacker: Pokemon, defender: Pokemon, move: Move, field?: BattleFieldStatus) {
    // ====== Terrain Related
    if(
        // Hydro Steam
    ...
    // electric ball
    if(move.id === 486) {
        const attackerSpeed = speedModifier(attacker.stat.speed, attacker.itemId, attacker.statStage.speed)
        const defenderSpeed = speedModifier(defender.stat.speed, defender.itemId, defender.statStage.speed)
        
        if (attackerSpeed / defenderSpeed >= 4) {
            return 150;
        } else {
            return 20 + Math.floor(attackerSpeed / defenderSpeed) * 3;
        }    
    }
    // Gyro ball
    if(move.id === 360) {
        const attackerSpeed = speedModifier(attacker.stat.speed, attacker.itemId, attacker.statStage.speed)
        const defenderSpeed = speedModifier(defender.stat.speed, defender.itemId, defender.statStage.speed)
        return Math.min(Math.trunc((25 * defenderSpeed) / attackerSpeed) + 1, 150)
    }
    // ====== Weight related
    // grass knot & low kick
    ...
    if(move.id === 500 || move.id === 681) {
        const counters = Object.values(attacker.statStage).filter(stage => stage > 0).reduce((pre, cur) => {
            return pre + cur
        }, 0)
...
function speedModifier(baseSpeed: number, itemId: number, stage: number): number {
    ...
    const itemModifier = 
        ...
        itemId === 1 ? 1.5
            ...
            : itemId === 2 ? 0.5
                : 1
    return Math.round((Math.trunc(baseSpeed * stageMultiplier * (4096 * itemModifier)) / 4096) - 0.001)
}
